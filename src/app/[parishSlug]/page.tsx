...
type Props = { params: { parishSlug: string; sellerSlug: string } };

export default function SellerPage({ params }: Props) {
  const parish = getParishBySlug(params.parishSlug);
  const seller = getSeller(params.parishSlug, params.sellerSlug);
  ...
  return (
    <main className="min-h-dvh bg-gradient-to-b from-orange-50 via-amber-50 to-white">
      <div className="mx-auto max-w-6xl px-5 py-8">
        ...
        <Section title="Menu andalan" subtitle="Demo dulu, nanti nyambung DB.">
          <Carousel>
            {products.map((p) => (
              <ProductCard
                key={p.slug}
